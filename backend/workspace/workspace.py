"""Project-root workspace with private AgentFlow metadata and delta tracking."""
...
class Workspace:
    ...
    def __init__(self, project_path: str):
        ...
        self.brief_path = self.project_root / "AGENTFLOW.md"
    ...
    def brief(self) -> str:
        return self.brief_path.read_text(errors="replace") if self.brief_path.exists() else ""
    ...
    def _safe_project_path(self, filename: str) -> Path:
        ...
        if any(part in {".agentflow", ".git"} for part in relative.parts):
            raise ValueError(f"Cannot write AgentFlow metadata path: {filename}")
    ...
    def changed_context(self, agent: str, roles: Optional[list[str]] = None) -> str:
        ...
        if roles is None or "src" in roles:

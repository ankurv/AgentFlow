...
This document outlines the testing strategies and instructions for verifying the various components of the AgentFlow system.

## 1. MCP Server System

### Overview
AgentFlow supports the Model Context Protocol (MCP) to allow agents to interact with external tools and systems. We must ensure that the MCP client can properly connect to both stdio and HTTP/SSE MCP servers, parse their tools, and allow Claude agents to invoke them.
...
**Steps to test:**
1. Start the AgentFlow application and open the UI in your browser.
